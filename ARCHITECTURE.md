...
- **iOS rate re-sync** — a context's `sampleRate` is locked at creation, but iOS re-configures the hardware rate when the mic session starts (e.g. 48 kHz → 24 kHz on some routes) and feeds mic samples into a stale-rate context **unresampled** — listeners hear wrong-speed audio. `syncCtxToMicSession()` (iOS only) rebuilds the shared context after `getUserMedia` when its rate doesn't match the track's (`getSettings().sampleRate`), or can't be verified. Not done elsewhere: a track-vs-context rate difference is normal on other platforms and correctly resampled, and rebuilding would churn every hold.
