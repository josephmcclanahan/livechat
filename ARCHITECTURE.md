...
- **Capture context per hold** — `startLiveTx` creates a **fresh `AudioContext` after `getUserMedia` resolves** and closes it on release. A context's `sampleRate` is locked at creation, but iOS re-configures the hardware rate when the mic session starts (e.g. 48 kHz → 24 kHz on some routes); capturing through a context created earlier makes the worklet resample from the wrong input rate — listeners hear half/double-speed audio. The per-hold `addModule` costs a few ms.
...
- **Output routes through a media element** — rx gains connect into a `MediaStreamAudioDestinationNode` playing through a persistent, gesture-unlocked `<audio>` element (`liveOut`), **not** `ctx.destination`: iOS mutes Web Audio's output channel with the ring/silent switch (symptom: live audio silent until some clip plays through an `<audio>` element), while media-element playback ignores the switch — the same reason committed clips play through the unlocked `player`. `getLiveDest()` re-kicks `liveOut.play()` on every call since iOS pauses it when the audio session is interrupted.
