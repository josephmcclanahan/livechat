...
- **Live path (worklet µ-law)** — an `AudioWorklet` (`pcm-capture-worklet.js`) resamples mic audio to **mono 16 kHz G.711 µ-law** (1 byte/sample, telephone-grade) and posts 60 ms frames, which go out as **binary WS frames**. Containerless frames need no codec negotiation, so the same code runs on every browser — **iOS transmits and hears live audio like everything else**. ~16 KB/s upstream per talker; frames are **shed, not queued**, when the socket backs up (`ws.bufferedAmount` > 12 KB), because late live audio is worthless while the archive clip is lossless anyway.
...
- **Jitter buffer (smoothness ↔ snappiness)** — playback starts once `rxLead` (150 ms floor) of audio is pre-buffered. On underrun mid-stream, `rxLead` grows toward the size of the stall just seen (`deficit×0.5 + 150 ms`, and at least ×1.5) up to a **1.2 s cap** — a fixed ×1.5 creep never catches a cellular link whose stalls run 300 ms–1 s+; growing toward the observed deficit covers the *next* similar stall. Playback then resumes **proportionally to the deficit** (60 ms floor, capped at the grown lead) — a 10 ms blip shouldn't insert a 225 ms gap, but re-anchoring tighter than the jitter that caused the underrun makes playback underrun every couple of frames (shipped once as persistent garble). The grown lead **persists across transmissions** and only decays (×0.8) when a stream plays clean (`lastRxUnderran` gates it), so a bad connection stays deeply buffered hold-to-hold while a good one tightens back to the 150 ms floor — the high cap costs latency only where smooth-but-late beats choppy, and the committed clip lands regardless. Each healthy new transmission decays the lead ×0.9 back toward the floor. A **playout servo** steers the backlog toward `rxLead` by scheduling each buffer up to ±3 % fast/slow (inaudible for voice): sender and listener clocks always drift a little, and un-absorbed drift drains the buffer into a glitch every few seconds. Debug state lives on `window.__lc` (`framesSent/Heard/Dropped`, `underruns`, `underrunLog`, `rxLead`, `rxRate`, `rxCodec`, `txRatio`, `txStepScale`, `txCal`).
